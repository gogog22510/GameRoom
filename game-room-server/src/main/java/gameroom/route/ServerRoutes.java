...
import gameroom.data.MissionUpdate;
import gameroom.mission.MissionManager;
import gameroom.utils.ActorDSL;
...
public class ServerRoutes extends AllDirectives {
    ...
    public ServerRoutes(ActorSystem system, ServiceLocator locator,
                        Settings settings, MissionManager missionManager) {
        this.system = system;
    ...
    private Route getAck(String message) {
        return get(() -> complete("Hello "+ message));
    }

    private Route handleMission() {
        return path(PathMatchers.segment(), missionId ->
            concat(
                get(() -> {
                    // show mission result
                    LOG.info("Get mission result {}", missionId);
                    MissionResult result = missionManager.getResult(missionId);
                    if (result != null) {
                ...
                post(() -> {
                    // create mission
                    LOG.info("Create mission {}", missionId);
                    boolean ok = missionManager.joinMission(missionId);
                    if (ok) {
                ...
                put(() -> entity(Jackson.unmarshaller(objectMapper, MissionUpdate.class), missionUpdate -> {
                    // update mission
                    LOG.info("Receive update {} for {}", missionId, missionUpdate);
                    boolean ok = missionManager.updateMission(missionId, missionUpdate.isGo());
                    if (ok) {
                        return complete(StatusCodes.OK, "{}");
                    } else {
                        return complete(StatusCodes.BAD_REQUEST);
